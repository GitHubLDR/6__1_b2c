 ...
 *  
 */
package com.ldr.cockpits.setup;
...
import com.ldr.cockpits.constants.LdrCockpitsConstants;
 ...
 */
@SystemSetup(extension = LdrCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(LdrCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(LdrCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						LdrCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/ldrcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
